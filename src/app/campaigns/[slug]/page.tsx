...
const statusConfig = {
  active: { label: 'Live Now', color: 'bg-gitcoin-green text-text-primary' },
  upcoming: { label: 'Upcoming', color: 'bg-system-warning text-white' },
  completed: { label: 'Completed', color: 'bg-lichenpunk-slateGray text-white' },
}
...
export default async function CampaignDetailPage({ params }: PageProps) {
  ...
  return (
    <div className="min-h-screen bg-lichenpunk-offWhite">
      ...
      {/* Hero */}
      {campaign.heroImage && (
        <div className="h-64 md:h-80 bg-lichenpunk-lichen relative overflow-hidden">
          <img
        ...
        <div className="container-page py-8">
          <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-6">
            <div className="max-w-2xl">
              ...
              <p className="text-lg text-text-secondary mb-4">{campaign.tagline}</p>
              <p className="text-text-secondary">
                Organized by <span className="font-medium text-text-primary">{campaign.organizer}</span>
              </p>
            </div>
            ...
            <div className="flex flex-col gap-3">
              ...
              {timeRemaining && (
                <div className="flex items-center gap-2 text-lichenpunk-moss">
          ...
          <div className="grid lg:grid-cols-3 gap-8">
            ...
            <div className="lg:col-span-2 space-y-8">
              ...
              <div className="card">
                ...
                <p className="text-text-secondary whitespace-pre-line">{campaign.description}</p>
              </div>

              {/* Eligibility */}
              {campaign.eligibility && campaign.eligibility.length > 0 && (
                <div className="card">
                  <h2 className="text-xl font-semibold text-text-primary mb-4">Eligibility</h2>
                  <ul className="space-y-3">
                    {campaign.eligibility.map((item, i) => (
                      <li key={i} className="flex items-start gap-3 text-text-secondary">
                        <CheckCircle className="w-5 h-5 text-gitcoin-green flex-shrink-0 mt-0.5" />
                        {item}
                      </li>
                    ))}
                  </ul>
              ...
              {/* Results */}
              {campaign.results && (
                <div className="card bg-gitcoin-green/5 border-gitcoin-green">
                  <h2 className="text-xl font-semibold text-text-primary mb-6">Results</h2>
                  <div className="grid grid-cols-2 md:grid-cols-3 gap-6 mb-6">
                    <div>
                      <p className="text-3xl font-bold text-gitcoin-green">{campaign.results.projectsFunded}</p>
                      <p className="text-sm text-text-secondary">Projects Funded</p>
                    </div>
                    <div>
                      <p className="text-3xl font-bold text-gitcoin-green">{campaign.results.totalDistributed}</p>
                      <p className="text-sm text-text-secondary">Total Distributed</p>
                    </div>
                    {campaign.results.uniqueDonors && (
                      <div>
                        <p className="text-3xl font-bold text-gitcoin-green">{campaign.results.uniqueDonors.toLocaleString()}</p>
                        <p className="text-sm text-text-secondary">Unique Donors</p>
                      </div>
                    )}
                  </div>
                  {campaign.results.highlights.length > 0 && (
                    <>
                      <h3 className="font-medium text-text-primary mb-3">Highlights</h3>
                      <ul className="space-y-2">
                        {campaign.results.highlights.map((highlight, i) => (
                          <li key={i} className="flex items-start gap-2 text-text-secondary">
                            <span className="w-1.5 h-1.5 rounded-full bg-gitcoin-green mt-2" />
                            {highlight}
                          </li>
                        ))}
                      </ul>
            ...
            <div className="space-y-6">
              {/* Key Info */}
              <div className="card">
                <h3 className="font-semibold text-text-primary mb-4">Key Information</h3>
                <dl className="space-y-4">
                  <div className="flex items-start gap-3">
                    <Calendar className="w-5 h-5 text-lichenpunk-moss flex-shrink-0" />
                    <div>
                  ...
                  {(campaign.matchingPool || campaign.fundingPool) && (
                    <div className="flex items-start gap-3">
                      <Coins className="w-5 h-5 text-lichenpunk-moss flex-shrink-0" />
                      <div>
                        <dt className="text-sm text-text-secondary">
                          {campaign.matchingPool ? 'Matching Pool' : 'Funding Pool'}
                        </dt>
                        <dd className="font-medium text-gitcoin-green">
                          {campaign.matchingPool || campaign.fundingPool}
                        </dd>
                      </div>
                    </div>
                  ...
                  {mechanism && (
                    <div className="flex items-start gap-3">
                      <Users className="w-5 h-5 text-lichenpunk-moss flex-shrink-0" />
                      <div>
              ...
              {campaign.caseStudy && (
                <div className="card bg-solarpunk-lightStone border-none">
                  <h3 className="font-semibold text-text-primary mb-2">Learn More</h3>
                  <p className="text-sm text-text-secondary mb-3">
                    Read the detailed case study for this campaign.
                  </p>
                  <Link
                    href={`/case-studies/${campaign.caseStudy}`}
                    className="text-lichenpunk-moss hover:text-lichenpunk-lichen font-medium text-sm"
                  >
                    View Case Study →
                  </Link>
                </div>
