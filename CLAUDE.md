...
> `authors` is optional. Names must exactly match entries in `src/data/authors.json`. Add new authors to that file before referencing them. Validation errors on unknown authors block CI.
>
...
**Before creating the file, always ask:** "Who is the author of this article?" If the user doesn't know or it's Gitcoin Research, omit the `authors:` field (the UI falls back to "Gitcoin Research" automatically). If an author is named, check `src/data/authors.json` — add a new entry if they're not listed yet, then set `authors:` in the frontmatter.
